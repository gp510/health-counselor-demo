"""
End-to-end tests for Wearable Health Data flow.
...
5. Real-time wearable data queries via the agent mesh (requires running system)

Usage:
    # Unit tests (no external dependencies)
    pytest tests/test_wearable_e2e.py -v -k "not requires_gateway"

    # Full E2E tests (requires running agents)
...
# ============================================================================
# E2E Tests requiring running system
# ============================================================================

class TestWearableE2EQueries:
    """E2E tests for wearable data queries via agent mesh.

    These tests require the full system to be running:
    - Solace broker
    - Health orchestrator
    - Fitness agent (for wearable data queries)
    - WebUI gateway
    """

    @pytest.mark.asyncio
    @pytest.mark.requires_gateway
    async def test_query_recent_heart_rate(self, async_http_client):
        """
        Test: "What is my current heart rate?"

        Expected: Response should mention heart rate data from fitness database.
        """
        from tests.conftest import send_query_and_get_response

        response = await send_query_and_get_response(
            async_http_client,
            "What is my current or most recent heart rate?",
            timeout=90.0
        )

        response_lower = response.lower()

        # Should mention heart rate
        assert any([
            "heart rate" in response_lower,
            "bpm" in response_lower,
            "heartbeat" in response_lower,
            "pulse" in response_lower,
        ]), f"Response should discuss heart rate. Got: {response[:500]}"

        print(f"\n--- Heart Rate Query Response ---\n{response[:1000]}")

    @pytest.mark.asyncio
    @pytest.mark.requires_gateway
    async def test_query_recent_steps(self, async_http_client):
        """
        Test: "How many steps have I taken today?"

        Expected: Response should include step count data.
        """
        from tests.conftest import send_query_and_get_response

        response = await send_query_and_get_response(
            async_http_client,
            "How many steps have I taken recently?",
            timeout=90.0
        )
        ...
        ]), f"Response should discuss steps. Got: {response[:500]}"

        print(f"\n--- Steps Query Response ---\n{response[:1000]}")

    @pytest.mark.asyncio
    @pytest.mark.requires_gateway
    async def test_query_workout_summary(self, async_http_client):
        """
        Test: "Summarize my recent workouts"

        Expected: Response should include workout types and durations.
        """
        from tests.conftest import send_query_and_get_response
        ...
        assert any([
            ...
            "exercise" in response_lower,
            "running" in response_lower,
            "cycling" in response_lower,
            "strength" in response_lower,
        ]), f"Response should discuss workouts. Got: {response[:500]}"

        print(f"\n--- Workout Summary Response ---\n{response[:1000]}")

    @pytest.mark.asyncio
    @pytest.mark.requires_gateway
    async def test_query_sleep_quality(self, async_http_client):
        """
        Test: "How well have I been sleeping?"

        Expected: Response should include sleep hours and quality metrics.
        """
        from tests.conftest import send_query_and_get_response

        response = await send_query_and_get_response(
            async_http_client,
            "How well have I been sleeping this week?",
            timeout=90.0
        )
        ...
        assert any([
            ...
            "hour" in response_lower,
            "rest" in response_lower,
            "quality" in response_lower,
        ]), f"Response should discuss sleep. Got: {response[:500]}"

        print(f"\n--- Sleep Quality Response ---\n{response[:1000]}")

    @pytest.mark.asyncio
    @pytest.mark.requires_gateway
    async def test_query_fitness_trends(self, async_http_client):
        """
        Test: "How is my fitness trending?"

        Expected: Response should analyze multiple fitness metrics over time.
        """
        from tests.conftest import send_query_and_get_response

        response = await send_query_and_get_response(
            async_http_client,
            "How is my overall fitness trending? Am I improving?",
            timeout=120.0
        )
        ...
        metrics_mentioned = sum([
            "step" in response_lower,
            "sleep" in response_lower,
            "heart" in response_lower,
            "workout" in response_lower or "exercise" in response_lower,
            "calorie" in response_lower,
        ])

        assert metrics_mentioned >= 2, (
            f"Expected multiple fitness metrics analysis. Got: {response[:500]}"
        )
    ...
    @pytest.mark.asyncio
    @pytest.mark.requires_gateway
    async def test_query_activity_vs_mood_correlation(self, async_http_client):
        """
        Test: "Does exercise affect my mood?"
        ...
        response = await send_query_and_get_response(
            async_http_client,
            "Is there a correlation between my exercise and my mood?",
            timeout=120.0
        )

        response_lower = response.lower()

        # Should mention both fitness and mood domains
        assert any([
            "exercise" in response_lower or "workout" in response_lower or "activity" in response_lower,
        ]), f"Response should mention exercise. Got: {response[:500]}"

        assert any([
            "mood" in response_lower or "feel" in response_lower or "mental" in response_lower,
        ]), f"Response should mention mood. Got: {response[:500]}"

        print(f"\n--- Activity-Mood Correlation Response ---\n{response[:1500]}")
